...
# starea initiala - la pornirea routerului
class StartUp(State):
    ...
    def execute(self):
        ...
        for i in addr:
            ...
            for k in self.fsm.router.routing_table:
                if k != i:
                    entry = RIPRouteEntry(address=self.fsm.router.routing_table[k].addr,nexthop=self.fsm.router.routing_table[k].nexthop,
    ...
    def setup_routing_table(self):
        addresses = self.fsm.router.get_inet_ips()

        for adr in addresses:
            net = self.fsm.router.calculate_network(adr,'255.255.255.0')
            self.fsm.router.routing_table[net] = RIPRouteEntry(address=net,nexthop=adr,metric=0,mask='255.255.255.0',imported=True)
...
class Waiting(State):
    ...
    def execute(self):
        ...
        data, State.addr = self.fsm.router.multicast_socket.recvfrom(8192)
