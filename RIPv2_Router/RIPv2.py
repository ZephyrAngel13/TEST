"""
    Clasele care descriu RIPv2: Header / Packet / RouteEntry
"""
import datetime
import struct
from socket import *
...
class RIPHeader:
    """ Format pachet:
    1 - un octet pentru comanda
    2 - un octet pentru versiune(implicit 2)
    3    - doi octeti de zero
    """
    FORMAT = "!BBH"
...
class RIPRouteEntry:
    """
    Formatul unui pachet:
    1- 2 octeti familia de adrese(implicit IPv4)
    2 - 2 octeti tagul rutei
    3 - 4 octeti adresa ip sursa
    4 - 4 octeti masca retelei
    5 - 4 octeti adresa ip a urmatorului hop
    """
    ...
    def __repr__(self):
        ...
        if self.timeout is None:
            return template.format(str(self.addr), str(self.metric), str(self.nexthop), str(self.changed),
                                   str(self.garbage), str(self.timeout))
        else:
            timeout = (datetime.datetime.now() - self.timeout).total_seconds()
            return template.format(str(self.addr), str(self.metric), str(self.nexthop), str(self.changed),
                                   str(self.garbage), str(round(timeout, 1)))
